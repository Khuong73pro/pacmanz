...
public abstract class GhostState {
	...
	public void computeNextDir() {
		...
		if (!ghost.onTheGrid())
			return; // Le fantôme doit être sur une "case" de la zone de jeu
		if (!ghost.onGameplayWindow())
			return; // Le fantôme doit être dans la zone de jeu

		double minDist = Double.MAX_VALUE; // distance minimale courante entre le fantôme et la cible selon sa prochaine
											// direction

		// Si le fantôme va actuellement vers la gauche et qu'il n'y a pas de mur à
		// gauche...
		if (ghost.getxSpd() <= 0 && !WallCollisionDetector.checkWallCollision(ghost, -ghost.getSpd(), 0)) {
			// On regarde la distance entre la position ciblée et la position potentielle du
			// fantôme si ce dernier irait vers la gauche
			double distance = Utils.getDistance(ghost.getxPos() - ghost.getSpd(), ghost.getyPos(),
					getTargetPosition()[0], getTargetPosition()[1]);

			// Si cette distance est inférieure à la distance minimale courante, on dit que
			// le fantôme va vers la gauche et on met à jour la distance minimale
			if (distance < minDist) {
		...
		// Une fois tous les cas testés, on change la direction du fantôme (au cas où,
		// comme cette direction est définie par une vitesse horizontale et une vitesse
		// verticale, on fait quand même une vérification afin qu'il ne puisse pas aller
		// en diagonale)
