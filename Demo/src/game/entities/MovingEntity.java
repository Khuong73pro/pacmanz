...
public class MovingEntity extends Entity {
	...
	public void updatePosition() {
		...
		if (!(xSpd == 0 && ySpd == 0)) { // Si la vitesse horizontale ou la vitesse verticale n'est pas nulle, on
											// incrémente la position horizontale et verticale en conséquence
			xPos += xSpd;
			yPos += ySpd;

			// En fonction de la direction emprunté, on change la valeur de la direction (un
			// entier permettant de savoir la partie de l'image à afficher notamment)
			if (xSpd > 0) {
				direction = 0;
			} else if (xSpd < 0) {
				direction = 1;
			...
			// On incrémente la valeur de l'image courante de l'animation à afficher (la
			// vitesse peut varier), et selon le nombre d'images de l'animation, la valeur
			// fait une boucle
			subimage += imageSpd;
		...
		// Si l'entité va au dela des bords de la zone de jeu, elle passe de l'autre
		// côté
	...
	public void render(Graphics2D g) {
		// Par défaut, on considère que chaque "sprite" contient 4 variations de
		// l'animation correspondant à une direction et chaque animation a un certain
		// nombre d'images
		// En sachant cela, on affiche seulement la partie de l'image du sprite
		// correspondant à la bonne direction et à la bonne frame de l'animation
		g.drawImage(sprite.getSubimage((int) subimage * size + direction * size * nbSubimagesPerCycle, 0, size, size),
				this.xPos, this.yPos, null);
	}

	// Méthode pour savoir si l'entité est bien positionnée sur une case de la
	// grille de la zone de jeu ou non
