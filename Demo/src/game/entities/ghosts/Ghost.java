...
public abstract class Ghost extends MovingEntity {
	...
	public void update() {
		if (!Game.getFirstInput())
			return; // Les fantômes ne bougent pas tant que le joueur n'a pas bougé

		// Si le fantôme est dans l'état effrayé, un timer de 7s se lance, et l'état
		// sera notifié ensuite afin d'appliquer la transition adéquate
		...
		// Si le fantôme est dans l'état chaseMode ou scatterMode, un timer se lance, et
		// au bout de 5s ou 20s selon l'état, l'état est notifié ensuite afin
		// d'appliquer la transition adéquate
		...
		// Si le fantôme est sur la case juste au dessus de sa maison, l'état est
		// notifié afin d'appliquer la transition adéquate
		if (xPos == 208 && yPos == 168) {
			state.outsideHouse();
		}

		// Si le fantôme est sur la case au milieu sa maison, l'état est notifié afin
		// d'appliquer la transition adéquate
		if (xPos == 208 && yPos == 200) {
			state.insideHouse();
		}

		// Selon l'état, le fantôme calcule sa prochaine direction, et sa position est
		// ensuite mise à jour
	...
	public void render(Graphics2D g) {
		// Différents sprites sont utilisés selon l'état du fantôme (après réflexion, il
		// aurait peut être été plus judicieux de faire une méthode "render" dans
		// GhostState)
		if (state == frightenedMode) {
